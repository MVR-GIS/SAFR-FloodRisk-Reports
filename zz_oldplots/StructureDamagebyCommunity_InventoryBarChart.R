...
#' 
StructureDamagebyCommunity_InventoryBarChart <- function(community_losses, communityname) {
  ...
  ggplot(CommunityStructuresByFrequency, mapping = aes(x=Scenario_Frequency)) +
    geom_col(aes(x=Scenario_Frequency, y=Structureinventorylossbyfrequency/1000)) +
    xlab("Frequency") +
    ylab("Inventory Loss in USD ($ ,000)") +
