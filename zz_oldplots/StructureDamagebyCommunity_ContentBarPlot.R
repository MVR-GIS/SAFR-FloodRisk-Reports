...
#' 
StructureDamagebyCommunity_ContentBarPlot <- function(community_losses, communityname) {
  ...
  ggplot(CommunityStructuresByFrequency, mapping = aes(x=Scenario_Frequency)) +
    geom_col(aes(x=Scenario_Frequency, y=StructurecontentLossbyfrequency/1000)) +
    xlab("Frequency") +
    ylab("Content Loss in USD ($ ,000)") +
