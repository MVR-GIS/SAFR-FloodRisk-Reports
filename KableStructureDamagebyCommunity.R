#' @description Creates a table of damages by community.
#' 
...
#' 
KableStructureDamagebyCommunity <- function(community_losses, communityname) {
  ...
  CommunityStructuresByFrequencyForPlot <- community_losses %>%
    filter(Total_Loss > 0) %>%
  ...
  kable(CommunityStructuresByFrequencyForPlot, 
        format.args = list(big.mark = ","),
        col.names = c("Frequency", "Total Loss",
                      "Building Loss",
                      "Content Loss", "Inventory Loss", "Structure Count"),
        caption = paste(unique(community_losses$StudyCity), "Losses by Frequency in USD"), 
        align = NULL) %>%
